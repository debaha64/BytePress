...
""",
    ...
    def test_optional_root_license_passes_and_unknown_root_path_fails(self):
        repo = self.copy_unit()
        (repo / "LICENSE").write_text("Copyright (c) 2026 Example\n", encoding="utf-8")
        self.assert_pass(self.run_checker(repo))

        (repo / "unknown-root-path").write_text("unexpected\n", encoding="utf-8")
    ...
    def test_existing_product_discovery_allows_preexisting_product_outside_surfaces(self):
        ...
        plan_numbers = []
        for plans_dir in (repo / "plans" / "active", repo / "plans" / "completed"):
            for candidate in plans_dir.glob("PLAN-*.md"):
                match = re.match(r"^PLAN-(\d+)(?:-|\.md$)", candidate.name)
                if match:
                    plan_numbers.append(int(match.group(1)))
        plan_id = f"PLAN-{max(plan_numbers, default=0) + 1:06d}"
        plan = repo / "plans" / "active" / f"{plan_id}-existing-product-discovery.md"
        plan.write_text(
            f"""# {plan_id}-existing-product-discovery
