...
3. Контракт роли ссылается на процедуры, но не владеет SOP, WPLAN или SYSTEM-инвариантами и не расширяет их разрешения.
...
21. `21 Вывод из эксплуатации` — [Decommissioning Engineer](21-decommissioning-engineer.md), конечное состояние: `retired`.

## Передача полномочий

Роль начинает изменения только когда активный WPLAN называет её `FROM_ROLE` с `FROM_ROLE_AUTHORITY: active` либо завершённая передача результата называет её `TO_ROLE` с `TO_ROLE_AUTHORITY: granted`. `withheld` запрещает следующую работу. После `relinquished` прежняя роль не сохраняет полномочия на запись; совместная работа требует отдельного явного исключения из контракта. Файл роли не выдаёт полномочия самостоятельно.
