...
## Индекс
- CHG-000101 — Завершающий проход перед выпуском 0.2.1: интервью, запрет догадок, язык и проверки
...
Источник: PLAN-000089
Связи: ROAD-000036, BACK-000101, QL-000096
Дата_создания: 2026-05-02
Дата_изменения: 2026-05-02

### Описание
`Docs/Discovery/Interview.md`, `Pipeline/Workflows.md`, `Pipeline/Phase_Gates.md`, `Pipeline/Inputs_Outputs.md`, `Pipeline/Artifacts.md`, `Rules/Git.md`, `Rules/Domains.md`, `Rules/Workflow.md`, `Docs/Discovery/README.md`, `Docs/User/Pass_Request.md`, `Templates/Interview.md`, `AGENTS.md`, `Tools/bp_bootstrap.py` и `Tools/bp_lint.py` синхронизированы вокруг упрощённого интервью, запрета на догадки агента и языковой чистки. Стартовое интервью сокращено до 7 блокирующих вопросов по классам `Результат`, `Граница`, `Вне границы`, `Ограничение`, `Стек и зависимости`, `Владение`, `Блокеры`; таймер, счётчик, несколько сценариев и любые расширения первой версии убраны из примеров. Запрет на догадки агента закреплён в `AGENTS.md`, `Pipeline/Workflows.md` и `Interview.md`: агент не предлагает дополнительный функциональный состав первой версии как пример; гипотезы фиксируются отдельно; пример результата использует только слова пользователя. Языковая чистка ключевых мест активного и создаваемого слоя: `product-start pass` → `проход`, `workflow` → `рабочий поток`, `gate` → `гейт`, `generated` → `создаваемый`; английский оставлен для имён файлов, каталогов, команд, веток, идентификаторов и имён собственных. Условия перед выпуском `0.2.1` зафиксированы.

### Эффект
Стартовое интервью стало короче и строже без расширяющих подсказок; запрет на догадки агента проверяется в трёх документах-владельцах и двумя инструментами; создаваемый продукт получает упрощённое интервью, русский канон в Pipeline и прямой запрет на расширение первой версии; `Minesweeper` не изменялся; новые домены не добавлялись.
