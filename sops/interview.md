...
`SOT_MODE` не является слоем или вопросом продуктового интервью. Устойчивый режим читается из `AGENTS.md`; начальная настройка отсутствующего репозитория завершается до интервью. Переход и изменяющая локальная Git-работа требуют отдельных типизированных решений по `sops/sot.md`.
...
Оба диапазона принадлежат тому же необработанному файлу и текущему `SESSION_ID`; полный `PRODUCT_INPUT` расположен раньше выбора `B`. Узкое исключение действует только для принятого продукта, новой задачи и перехода `sot_files -> sot_git`: `PLAN_ID` переходного IE указывает переходный PLAN, созданный из того же выбора `B` и впервые зафиксированный вместе с IE и обеими OD в одном коммите перехода. Не утверждается, что этот PLAN существовал или был активным при получении `PRODUCT_INPUT` либо `B`; время и порядок входа владельца доказывают `PRODUCT_INPUT_REF`, `ROUTE_CHOICE_REF`, `SOURCE_REF` и `SESSION_ID`. Переходный IE закрывает пять продуктовых слоёв и `next-transition`, но не открывает реализацию. Следующий PLAN функционального изменения или стартового исследования (`discovery`) ссылается на этот IE без создания второй записи и без смены `EVIDENCE_CLASS`.
...
Для действительной точки контроля обычный `IE.PLAN_ID` остаётся PLAN фиксации; у переиспользуемого переходного IE он остаётся переходным PLAN той контрольной отметки. `OD.PLAN_ID` реализации совпадает с активным PLAN реализации. `EVIDENCE_REF` решения и `INTERVIEW_EVIDENCE_REF` PLAN реализации ссылаются на один действительный `IE-*`. Дополнительные поля `BASIS_PLAN_ID` и `CAPTURE_PLAN_ID` не используются.
...
5. `OD-*` и ссылка активного PLAN реализации фиксируются вместе с отдельной контрольной отметкой `implementation-open`.
...
2. в поле `Product Unit` PLAN стартового исследования;
3. в IE и маршруте текущего BACK;
4. в закрытии стартового исследования или подготовленной следующей точке решения.

Переименование `## Продукт` в `## Название` не задаёт идентичность. Последующие PLAN режима `product-work` и README с приоритетом продукта используют то же точное имя.
