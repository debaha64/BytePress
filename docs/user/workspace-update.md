...
Исполнитель запускается из новой проверенной distribution. Обычный Update выполняется на non-executing checkpoint без active WPLAN; подготовительный research WPLAN под старым checker не открывается. Если конкретную разрешённую работу блокирует Harness defect, допускается узкий [recovery с сохранением active WPLAN](../../sops/change-management.md#recovery-при-active-wplan): остановить записи, подготовить exact backup и воспроизводимый blocker, отдельно разрешить точные изменения Harness. Product, маршрут, research, журналы и Feedback сохраняются. Active WPLAN не нужно закрывать ради обновления. Владелец отдельно разрешает exact backup/digest, новую distribution и полный список изменений после анализа различий. Исполнитель сопоставляет новую поставку с рабочей средой, обновляет только разрешённые поверхности и проверяет сохранность продукта и истории. Нормативная процедура принадлежит [change-management](../../sops/change-management.md#workspace-update).
