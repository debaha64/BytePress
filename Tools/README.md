...
- проверку минимального bootstrap-contract fresh product repo;
- проверку structural consistency developed product repo после подтверждения current truth и закрытого first pass;
...
### Режимы product repo
Для product repo `bp_lint.py` поддерживает:
- `--mode product-fresh` — строгая проверка состояния сразу после `bp_bootstrap.py`: `Статус_текущей_истины: Не_подтверждена`, placeholders интервью, `ROAD-000001`, `BACK-000001` и `PLAN-000001` в статусе `В_работе`;
- `--mode product-developed` — проверка развивающегося продукта после подтверждения current truth: `Docs/Discovery/Interview.md` подтверждён, first pass закрыт в `Plans/*`, следующий active или completed plan есть, а `Logs/*` содержит closure facts;
- `--mode auto` — default route для generated `scripts/dev-test.sh`; режим выбирается по current truth status.
...
- `python3 Tools/bp_lint.py --repo <product-repo> --mode product-fresh`.
- `python3 Tools/bp_lint.py --repo <product-repo> --mode product-developed`.
