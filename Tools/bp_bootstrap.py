...
def bootstrap_product(target: Path, ctx: ProductContext) -> None:
    ...
    write(
        ...
        "## Gating первого product-start pass\n"
        "- Первый product-start pass остаётся discovery-only: агент проводит интервью и не начинает implementation.\n"
        "- Агент не самозаполняет discovery interview и не подменяет unanswered вопросы своими догадками.\n"
        "- До ответов пользователя на discovery interview агент не имеет права менять product truth, planning, logs и code.\n"
        "- Первый write-pass продукта запускается только после подтверждённого discovery и только через task-ветку; напрямую в `main` / `develop` писать запрещено.\n\n"
    ...
    write(
        ...
        "- unanswered вопросы не самозаполняются агентом и не подменяются его догадками;\n"
        "- до ответов пользователя discovery-layer не переводится в write-state: нельзя менять product truth, planning, logs и code;\n"
        "- первый write-pass допускается только после подтверждённого discovery и отдельного task-веточного pass;\n"
    ...
    write(
        ...
        "Unanswered вопросы остаются unanswered до ответа пользователя: агент не самозаполняет их и не начинает write-pass по догадке.\n\n"
        "---\n\n"
        "## Вопросы и ответы\n\n"
        ...
        "Ответ: Открыть отдельный первый write-pass через task-ветку и только после этого менять product truth, planning, logs и code.\n\n"
        "До ответов пользователя на это интервью агент не имеет права менять product truth, planning, logs и code.\n\n"
        "---\n",
    )
    write(
    ...
    write(
        ...
        "# Roadmap\n\n"
        "## Индекс\n"
        "- ROAD-000001 — Подтвердить discovery продукта и подготовить первый write-pass\n\n"
        "## Легенда статусов\n"
        "- Черновик\n- Готово_к_утверждению\n- Утверждено\n- В_работе\n- Завершено\n- Отменено\n\n"
        "---\n\n"
        "## ROAD-000001 — Подтвердить discovery продукта и подготовить первый write-pass\n"
        "ID: ROAD-000001\n"
        "Этап: Подтвердить discovery продукта и подготовить первый write-pass\n"
        "Статус: В_работе\n"
        "Связи: BACK-000001\n"
        "Источник: PLAN-000001\n"
        f"Дата_создания: {ctx.current_date}\n"
        f"Дата_изменения: {ctx.current_date}\n"
        "Цель: Провести discovery-only первый pass продукта, зафиксировать ответы пользователя и подготовить отдельный task-веточный route первого write-pass без раннего implementation.\n"
        "Зависимости:\n"
        "Связанные_backlog: BACK-000001\n\n"
        "### Описание\n"
        "Bootstrap уже materialize first-usable replicated repo, поэтому первый текущий pass остаётся discovery/planning contour: агент даёт startup-handshake, проводит discovery-only interview, не самозаполняет ответы и не начинает writes до подтверждённого discovery. Первый write-pass продукта открывается только следующим отдельным pass через task-ветку.\n",
    )
    write(
        ...
        "## ROAD-000001 — Подтвердить discovery продукта и подготовить первый write-pass\n\n"
        "### Активные\n\n"
        "#### BACK-000001 — Провести discovery-only interview и подготовить write route\n"
        "ID: BACK-000001\n"
        "Название: Провести discovery-only interview и подготовить write route\n"
        "Тип: Продукт\n"
        "Приоритет: Высокий\n"
        "Статус: В_работе\n"
        ...
        f"Дата_изменения: {ctx.current_date}\n\n"
        "##### Описание\n"
        "Дать startup-handshake, провести discovery-only interview, дождаться ответов пользователя и подготовить отдельный task-веточный route первого write-pass. До ответов пользователя агент не самозаполняет интервью и не меняет product truth, planning, logs или code.\n\n"
        "### Завершённые\n"
        "- отсутствуют\n\n"
    ...
    write(
        ...
        "ID: PLAN-000001\n"
        "Название: Product start discovery and first write-pass gating\n"
        "Статус: В_работе\n"
        "Связи: BACK-000001\n"
        "Источник: First current pass replicated product repo\n"
        f"Дата_создания: {ctx.current_date}\n"
        f"Дата_изменения: {ctx.current_date}\n"
        "Основание: Bootstrap materialize first-usable replicated product repo; текущий pass остаётся discovery-only и должен подготовить первый write-pass без раннего implementation.\n"
        "Связанные_требования:\n"
        "Связанные_backlog: BACK-000001\n"
        "Связанные_ADR:\n\n"
        "## Шаги\n"
        "1. Дать обязательный startup-handshake и прочитать owner-contracts первого product-start pass.\n"
        "   - DoD: startup mode, scope, branch/start route, planning-state, owner-domains и первый конкретный шаг явно проговорены до дальнейших действий.\n"
        "2. Провести discovery-only interview.\n"
        "   - DoD: пользователь дал ответы на `Docs/Discovery/Interview.md`, а агент не самозаполнил unanswered вопросы.\n"
        "3. Подготовить первый write-pass продукта как отдельный route.\n"
        "   - DoD: до ответов пользователя агент не имеет права менять product truth, planning, logs и code; после подтверждённого discovery определён отдельный task-веточный route первого write-pass без прямых writes в `main` / `develop`.\n\n"
        "## Риски\n"
        "- агент может попытаться самозаполнить интервью и искусственно закрыть discovery gap;\n"
        "- ранний переход к implementation сломает source-of-truth contour первого запуска.\n\n"
        "## Артефакты\n"
        "- AGENTS.md\n- Docs/Discovery/*\n- Plans/*\n- Docs/User/*\n- Profiles/Product.md\n- scripts/*\n\n"
        "## DoD\n"
        "Первый product-start pass остаётся discovery-only, self-answering запрещён, write-pass отложен до подтверждённого discovery и отдельной task-ветки.\n",
    )
