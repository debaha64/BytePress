...
def bootstrap_product(target: Path, ctx: ProductContext) -> None:
    ...
    write(
        ...
        "   - Определение_готовности: первый стартовый проход продукта не идёт из `develop` или `main`.\n"
        ...
        "   - Определение_готовности: статус текущей истины больше не остаётся стартовой заготовкой, а свободноформатная замена структурированного выбора не используется.\n"
        "3. Синхронизировать аналитический контур с `Plans/*` и `Logs/*`.\n"
        "   - Определение_готовности: плановый и журнальный маршрут отражает подтверждённую текущую истину без выхода в продуктовые документы или реализацию.\n"
        "4. Открыть следующий предметный проход только после подтверждённой текущей истины.\n"
        "   - Определение_готовности: следующая область сформулирована отдельно и не смешана со стартовыми заготовками.\n\n"
        "## Риски\n"
