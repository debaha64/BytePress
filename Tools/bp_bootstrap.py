...
def check(root: Path, mode: str) -> list[str]:
    ...
    if not INTERVIEW_NO_GUESSED_EXPANSION.search(text(interview)):
        errors.append("Docs/Discovery/Interview.md: missing ban on agent offering additional first-version functionality as examples")
    if INTERVIEW_UNCONFIRMED_EXPANSION_WORDS.search(text(interview)):
        errors.append("Docs/Discovery/Interview.md: must not suggest unconfirmed first-version expansion examples like timer, counter, records, settings, saves, installer")
...
def bootstrap_product(target: Path, ctx: ProductContext) -> None:
    ...
    write(
        ...
        "- Гипотезы фиксируются отдельно и не становятся текущей истиной без явного подтверждения пользователем.\n"
    ...
    write(
        ...
        "- агент не предлагает дополнительный функциональный состав первой версии как пример, если пользователь его не назвал;\n"
        "- неявные предположения фиксируются отдельно как гипотезы; гипотезы не становятся текущей истиной без явного подтверждения пользователем;\n"
    ...
    write(
        ...
        "Правило: агент не предлагает дополнительный функциональный состав первой версии как пример, если пользователь его не назвал.\n"
        "Правило: неявные предположения фиксируются отдельно как гипотезы; гипотезы не становятся текущей истиной без явного подтверждения пользователем.\n"
        "Правило: пример результата использует только слова пользователя или уже подтверждённые требования.\n"
        ...
        "Правило: пример результата интервью использует только слова пользователя или уже подтверждённые требования.\n\n"
        "---\n\n"
