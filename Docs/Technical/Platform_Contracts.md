...
4. Commit message, PR title и PR body оформляются на английском языке.
5. `branch slug` оформляется на английском в формате `kebab-case`.
6. Язык user-facing взаимодействия определяется параметром профиля `Язык_взаимодействия`, а не языком Git/PR-артефактов.
