...
К ним относятся:
- `Runtime/*` как временный execution interface активного pass;
- `Tools/*` как service interface materialization и checks;
- `Templates/*` и `Schemas/*` как support interfaces формы и валидации;
- `Profiles/*`, `Roles/*`, `Rules/*`, `Standards/*` как governance interfaces настройки поведения.
...
### `Plans/*` -> `Runtime/*`
Утверждённый active `Plan` открывает временный execution interface.

Допустимый contract:
- runtime-context опирается на текущий `Plan`;
- runtime может хранить промежуточные заметки и рабочее состояние pass.

Недопустимо:
- создавать в runtime новый канонический план;
- переносить в runtime статус stage или task.

### `Runtime/*` -> `Logs/*`
Подтверждённый результат исполнения передаётся в fact-layer.

Допустимый contract:
- проверенный результат pass фиксируется в `Logs/*`;
- runtime служит только промежуточной точкой до log fixation.

Недопустимо:
- считать runtime самодостаточным фактом;
- оставлять результат только в runtime без записи в журналы, если pass реально изменил систему.
...
### `Profiles/*` -> `Roles/*`, `Rules/*`, `Standards/*`
Профиль задаёт активный operating mode через governance interfaces.

Допустимый contract:
- profile references указывают canonical `ROLE-*`, `RULE-*`, `STD-*` и другие governance IDs;
- profile активирует режим поведения, но не меняет технические contracts напрямую.

Недопустимо:
- встраивать в profile свободнотекстовые заменители canonical links;
- использовать profile как substitute для rule или standard.

### `Roles/*` -> `Skills/*`, `Rules/*`, `Standards/*`
Role связывает поведенческий сценарий с навыками и governance constraints.

Допустимый contract:
- role references указывают конкретные skills, rules и standards;
- роль задаёт поведенческую сборку, не владея чужим состоянием.

Недопустимо:
- переносить в `Roles/*` сами rules, standards или skills;
- использовать role как место хранения process-canon или planning-state.
