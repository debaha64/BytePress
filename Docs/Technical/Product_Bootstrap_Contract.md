...
До отдельного tool-migration pass текущий `bp_bootstrap.py` всё ещё materialize прежний baseline. Этот документ удерживает both states: текущий implemented contract ниже и целевой refactoring direction через `Product_Bootstrap_Domain_Matrix.md` и `Domain_Model_Migration_Plan.md`.
...
- что первое записываемое действие generated product repo допускается только после открытия task-ветки, включая `Docs/Discovery/*`, `Plans/*` и `Logs/*`;
...
### Target product tools layer
В целевой профильной модели bootstrap обязан создавать локальный product `Tools/*`, который заменяет зависимость generated product repo от `BYTEPRESS_ROOT`.

Целевой product `Tools/*` обязан:
- запускать structural check продукта без обращения к `BytePress`;
- проверять только packages, выбранные профилем;
- хранить временные отчёты в ignored tool-output path;
- оставаться частью product repo, а не внешней service dependency.

Этот target layer не реализован текущим `bp_bootstrap.py` и должен быть введён отдельным migration pass.
...
Минимальный путь для `scripts/dev-test.sh` и `scripts/README.md`:
1. взять основание из текущего `BytePress` contract и фактического generated skeleton;
2. открыть task-ветку внутри product repo;
3. перенести только служебный delta в `scripts/dev-test.sh` и `scripts/README.md`;
4. не пересоздавать product repo и не переписывать `Docs/Product/*`, `Docs/Discovery/*`, `Plans/*`, `Logs/*` или предметный код вне прямой необходимости pass;
5. проверить product repo через `scripts/dev-test.sh` или `python3 <BytePress>/Tools/bp_lint.py --repo <product-repo> --mode auto`;
6. зафиксировать product-side planning/log closure.
...
- `scripts/dev-test.sh` запускает `bp_lint.py --mode auto`, поэтому repo проходит developed product gate без возврата к fresh bootstrap markers.

## Что bootstrap materialize только как каркас
Bootstrap создаёт каркас, но не завершённое содержательное состояние, для:
- `AGENTS.md` продукта — только minimal agent entry point replicated repo, а не копию `AGENTS.md` самого `BytePress`; при этом product `AGENTS.md` обязан требовать наблюдаемый стартовый отчёт первого ответа с режимом запуска, scope, статусом ветки, действием с веткой/стартовым маршрутом, planning-state, доменами-владельцами первого чтения и первым конкретным шагом;
- product `AGENTS.md` обязан требовать короткий стартовый отчёт блоком с приветствием и семью полями: режим запуска, scope, статус ветки, действие с веткой/стартовый маршрут, planning-state, домены-владельцы первого чтения и первый конкретный шаг;
...
После tool-migration pass этот список должен быть пересобран: lightweight `Pipeline/*`, profile-bound `Templates/*`, profile-bound `Schemas/*`, optional `Rules/*` и local `Tools/*` станут allowed product packages, а `Adapters/*`, `Memory/*`, `MCP/*`, `Runtime/*`, `Roles/*`, `Skills/*` и `Standards/*` останутся запрещёнными до появления реального mechanism contract.
...
- на migration stage держать старые generated `scripts/*`, пока product-local `Tools/*` не реализован и не проверяется.
...
- использовать bootstrap-заготовки как скрытое разрешение на изменение `Docs/Product/*`, `Docs/Technical/*`, `Runtime/*`, `scripts/*` или предметного кода до ответов пользователя;
...
`Product_Bootstrap_Domain_Matrix.md` задаёт target package composition. Если matrix и текущий `bp_bootstrap.py` расходятся во время migration stage, это считается known implementation gap до соответствующего tool pass, а не разрешением менять generated product вручную.
