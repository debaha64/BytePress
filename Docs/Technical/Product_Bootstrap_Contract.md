...
- product `AGENTS.md` обязан требовать короткий стартовый отчёт блоком с приветствием и семью полями: startup mode, scope, branch status, branch action/start route, planning-state, owner-domains первого чтения и первым конкретным шагом;
