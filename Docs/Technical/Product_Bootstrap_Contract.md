...
Bootstrap materialize отдельный product repo с first-usable initialized state, пригодным для первого discovery-only product-start pass и согласованным с current human/agent entry contour `BytePress`.
...
- что unanswered вопросы не самозаполняются агентом и не подменяются его догадками;
- что до ответов пользователя на discovery interview агент не меняет product truth, planning, logs и code;
- что первый write-pass продукта открывается только после подтверждённого discovery и отдельного task-веточного route.
...
- initial `Roadmap`, `Backlog` и `Plan` описывают discovery/planning contour первого product-start pass, а не немедленный implementation-pass;
- initial planning contour явно фиксирует, что первый write-pass продукта запускается только после ответов пользователя и через task-ветку, а не внутри bootstrap-created `PLAN-000001`.
...
- `AGENTS.md` продукта — явный hard-gate первого product-start pass: агент не самозаполняет discovery interview, не делает write-изменения до ответов пользователя и не пишет напрямую в `main` / `develop`;
- `AGENTS.md` продукта — прямые writes в `main` / `develop` запрещены даже после подтверждённого discovery; первый write-pass идёт только через task-ветку;
...
- создавать generated product `AGENTS.md` без явного запрета self-answering interview, write-изменений до ответов пользователя или writes в `main` / `develop`;
...
- строить initial `ROAD/BACK/PLAN` так, будто первый product-start pass уже обязан писать product truth или начинать implementation до подтверждённого discovery;
