...
- что начальный запрос пользователя и общий запрос вида «хочу сделать продукт» не считаются подтверждённой текущей истиной;
- что заготовки начального развёртывания не считаются подтверждённой текущей истиной;
- что если агент задал блокирующие вопросы, он останавливается и ждёт ответа пользователя;
- что до ответа пользователя агент не меняет `Docs/Discovery/Interview.md`, `Plans/*` и `Logs/*`, кроме случая явных структурированных ответов в первом сообщении;
...
Discovery minimum обязан также фиксировать зависимостные ограничения первого прохода:
...
- при отсутствии модуля агент фиксирует ограничение среды и не пытается установить системный пакет;
- `apt`, `apt-get`, `sudo apt`, `sudo apt-get` и аналоги не предлагаются как часть обычного первого прохода.

Первая версия не расширяется без явного ответа пользователя. Уровни сложности, таймер, рекорды, сохранения, темы, установщик, упаковка и многопользовательский режим не входят в первый проход, если пользователь не подтвердил их явно.
...
- product `AGENTS.md` обязан требовать остановку после блокирующего интервью: агент не продолжает работу, не заполняет ответы вместо пользователя, а первый записываемый проход начинается только после фактического ответа пользователя;
