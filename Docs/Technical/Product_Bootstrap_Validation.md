...
- bootstrap не materialize `Adapters/*`, `Memory/*`, `MCP/*`, `Runtime/*`, `Roles/*`, `Skills/*` или `Standards/*` в generated repo;
...
Служебное обновление уже созданного developed product repo валидируется как product-side pass, а не как новый bootstrap-result. Канонический маршрут описан в `Docs/Technical/Product_Service_Update_Route.md`. Для обновлений уровня `Tools/*`, lightweight `Pipeline/*`, bounded `Templates/*`/`Schemas/*` и transition wrappers `scripts/*` validating сторона проверяет, что:
