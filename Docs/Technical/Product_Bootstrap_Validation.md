...
- generated product `AGENTS.md` требует наблюдаемый startup-handshake первого ответа с mode, scope, route, planning-state, owner-domains первого чтения и первым конкретным шагом;
- generated product `AGENTS.md` удерживает hard first product-start gate: пока `Docs/Discovery/Interview.md` имеет `Статус_текущей_истины: Не_подтверждена`, agent остаётся в discovery-only contour и не трактует placeholders как разрешение на implementation;
- `Docs/Discovery/Interview.md` generated repo остаётся owner current truth и содержит 8–10 ключевых вопросов первого pass с буквенными вариантами там, где выбор ограничен, и рекомендуемым вариантом там, где он нужен;
...
- target repo получает hard first product-start gate в `AGENTS.md`, initial `Plan` и discovery-layer, а не только общую startup prose;
...
- наличие hard first product-start gate в generated product `AGENTS.md`;
- наличие в generated `Docs/Discovery/Interview.md` 8–10 ключевых вопросов, буквенных вариантов и рекомендуемого варианта там, где это уместно;
