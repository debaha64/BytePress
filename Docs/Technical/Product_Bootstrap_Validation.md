...
`Tools/bp_lint.py` автоматизирует часть structural validation, но не заменяет весь validation-contract.
...
- последующие продуктовые passes после initial product initialization;
...
- generated product `AGENTS.md` требует наблюдаемый startup-handshake первого ответа с mode, scope, branch status, branch action/start route, planning-state, owner-domains первого чтения и первым конкретным шагом;
- generated product `AGENTS.md` удерживает hard first product-start gate: пока `Docs/Discovery/Interview.md` имеет `Статус_текущей_истины: Не_подтверждена`, agent остаётся в discovery-only contour, не трактует placeholders как разрешение на implementation и не делает writable changes до открытия task-ветки;
- `Docs/Discovery/Interview.md` generated repo остаётся owner current truth и содержит 8–10 ключевых вопросов первого pass; и full interview, и допустимый delta-интервью используют нумерованные вопросы, буквенные варианты там, где выбор ограничен, и рекомендуемый вариант там, где он нужен;
- `Docs/Discovery/Interview.md` generated repo явно фиксирует `Статус_текущей_истины: Не_подтверждена` и запрет трактовать bootstrap placeholders как подтверждённую current truth;
...
- created repo проходит обязательный structural check текущего `bp_lint.py`;
...
- target repo получает hard first product-start gate в `AGENTS.md`, initial `Plan` и discovery-layer, а не только общую startup prose; этот gate требует task-ветку до любых writable changes, включая discovery/planning/log closure;
...
- bootstrap-result проходит machine-check уровня `python3 Tools/bp_lint.py --repo <product-repo>`;
...
- наличие observable startup-handshake contract в generated product `AGENTS.md`;
- наличие hard first product-start gate в generated product `AGENTS.md`, который требует task-ветку до любых writable changes;
- наличие в generated `Docs/Discovery/Interview.md` 8–10 ключевых вопросов, буквенных вариантов, рекомендуемого варианта там, где это уместно, и explicit delta-интервью contract в том же формате;
- наличие в generated `Docs/Discovery/Interview.md` статуса `Статус_текущей_истины: Не_подтверждена` и явного запрета трактовать bootstrap placeholders как подтверждённую current truth;
...
- created placeholders остаются first-usable replicated contour, а не случайным смешением bootstrap и последующего предметного наполнения;
...
- считать bootstrap placeholders или незаполненный discovery достаточным основанием для перехода к implementation;
