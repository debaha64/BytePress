...
- bootstrap не materialize `scripts/*`; все служебные команды нового каркаса идут через local `Tools/*`;
- создаются `Docs/Discovery/README.md` и `Docs/Discovery/Interview.md` как minimal current-truth route generated repo;
- generated product `AGENTS.md` требует наблюдаемый стартовый отчёт первого ответа с режимом запуска, областью, статусом ветки, действием с веткой/стартовым маршрутом, состоянием планирования, доменами-владельцами первого чтения и первым конкретным шагом;
...
- generated product `AGENTS.md` удерживает hard first product-start gate: пока `Docs/Discovery/Interview.md` имеет `Статус_текущей_истины: Не_подтверждена`, agent остаётся только в аналитическом контуре, не трактует bootstrap-заготовки как разрешение на implementation и не делает записываемые изменения до открытия рабочей ветки;
- generated product `AGENTS.md` требует остановки после блокирующего интервью: агент ждёт ответа пользователя, не заполняет ответы вместо пользователя и не начинает первый записываемый проход до фактического ответа;
- `Docs/Discovery/Interview.md` generated repo остаётся владельцем текущей истины и содержит 8–10 ключевых вопросов первого прохода; и полное интервью, и допустимое узкое интервью используют нумерованные вопросы, буквенные варианты там, где выбор ограничен, рекомендуемый вариант там, где он нужен, смысловые классы `Контекст`, `Граница`, `Ограничение`, `Владение`, `Переход` и правило `блокирующее сразу / неблокирующее в следующую фазу`;
- `Docs/Discovery/Interview.md` generated repo явно фиксирует `Статус_текущей_истины: Не_подтверждена`, запрет трактовать заготовки начального развёртывания как подтверждённую текущую истину и запрет превращать общий запрос вида «хочу сделать продукт» в подтверждённую текущую истину;
- generated тексты запрещают самовольный выбор `tkinter`, попытку установки системных пакетов и расширение первой версии уровнями сложности, таймером, рекордами, сохранениями, темами, установщиком, упаковкой или многопользовательским режимом без явного ответа пользователя;
...
- target repo получает гейт текущей истины в `AGENTS.md`, initial `Plan` и discovery-layer, а не только общую startup prose; этот гейт требует рабочую ветку с типом `chore/`, `feature/`, `fix/` или `docs/` до любых записываемых изменений, включая discovery/planning/log closure;
- target repo получает `Docs/Terms/Base_Terms.md` со стартовым пакетом терминов, достаточным для раннего product-start;
- target repo получает `.gitignore`, в котором `Tools/.reports/` остаётся local tool-output path и не попадает в baseline-фиксацию по умолчанию;
- bootstrap materialize минимальный technical subset продукта: `README.md`, `Architecture.md`, `Interfaces.md`, `System_Invariants.md`;
- bootstrap materialize product layer `Docs/Product/README.md`, `Product_Passport.md`, `JTBD.md`, `PRD.md`, `Delivery.md`, готовый к дальнейшей предметной жизни;
- `Docs/Product/Product_Passport.md` стартует как паспорт созданного каркаса и содержит места для предметного обновления: тип продукта, основной запуск, подтверждённый стек, предметные пакеты и ограничения среды;
...
- generated `Pipeline/Workflows.md` описывает основной путь продукта, первый старт продукта, предметный проход, гейты, уровни проверок, журнальное закрытие и маршрут запроса на слияние через `gh`;
...
- наличие гейта текущей истины в generated product `AGENTS.md`, который требует рабочую ветку с типом `chore/`, `feature/`, `fix/` или `docs/` до любых записываемых изменений;
- наличие в generated product `AGENTS.md` правила остановки после блокирующего интервью и запрета заполнять ответы вместо пользователя;
- наличие в generated `Docs/Discovery/Interview.md` 8–10 ключевых вопросов, буквенных вариантов, рекомендуемого варианта там, где это уместно, смысловых классов вопросов и правила узкого интервью в том же формате;
- наличие в generated `Docs/Discovery/Interview.md` статуса `Статус_текущей_истины: Не_подтверждена`, явного запрета трактовать стартовые заготовки как подтверждённую текущую истину, правила ожидания ответа после вопросов и запрета подтверждать общий запрос без ответов пользователя;
...
- наличие в generated текстах запрета самовольного выбора `tkinter`, системной установки через `apt`/`apt-get`/`sudo apt`/`sudo apt-get` и расширяющих функций первого прохода без явного ответа;
- наличие в generated `Pipeline/Workflows.md` маршрута запроса на слияние через `gh`;
- наличие `Tools/product_check.py` и его executable mode;
- наличие `Tools/product_bootstrap_smoke.py` и его executable mode;
- отсутствие `scripts/*` в fresh product repo нового образца;
- наличие `.gitignore`, который удерживает `Tools/.reports/` вне baseline-фиксации по умолчанию;
...
- `ROAD-000001`, `BACK-000001` и `PLAN-000001` имеют статус `Завершено` или `Готово_к_утверждению` как результат первого product-start pass;
- после `PLAN-000001` есть следующий active, ready-for-approval или completed plan;
...
Если код, документы и автоматические проверки готовы, но пользовательский результат или запуск нельзя проверить из-за ограничения среды, `ROAD/BACK/PLAN` не обязаны получать `Завершено`. В таком случае допустим статус `Готово_к_утверждению`, а `Logs/QualityLog.md` обязан явно записать выполненные проверки и непроверенную зону как `не проверено`.

Служебное обновление уже созданного developed product repo проверяется как product-side pass, а не как новый bootstrap-result. Канонический маршрут описан в `Docs/Technical/Product_Service_Update_Route.md`. Для обновлений уровня `Tools/*`, lightweight `Pipeline/*`, bounded `Templates/*`/`Schemas/*` и наследия `scripts/*` старых продуктов проверяющая сторона проверяет, что:
...
- считать выданные вопросы интервью основанием продолжать работу без ответа пользователя;
