...
- generated product `AGENTS.md` явно запрещает self-answering discovery interview, любые write-изменения до ответов пользователя и writes в `main` / `develop`;
...
- initial `Roadmap`, `Backlog` и `Plan` generated repo удерживают discovery-only / planning-first contour и не подталкивают к немедленному implementation-pass или early MVP build;
...
- target repo получает explicit gating contract discovery-only первого pass: user answers required before write-pass, self-answering запрещён, direct writes в `main` / `develop` запрещены;
- target repo до ответов пользователя не имеет права менять product truth, planning, logs и code;
...
- наличие в generated product `AGENTS.md` hard-gates: no self-answering, no writes before user answers, no direct writes in `main` / `develop`;
...
- наличие в generated `Docs/Discovery/*` и initial `Plans/*` явного discovery-only / write-later contour первого product-start pass;
