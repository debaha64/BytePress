...
Служебное обновление уже созданного developed product repo валидируется как product-side pass, а не как новый bootstrap-result. Для обновлений уровня `Tools/*` и transition wrappers `scripts/*` validating сторона проверяет, что:
