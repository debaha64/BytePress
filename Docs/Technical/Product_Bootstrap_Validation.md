...
- target repo получает гейт текущей истины в `AGENTS.md`, initial `Plan` и discovery-layer, а не только общую startup prose; этот гейт требует task-ветку с типом `chore/`, `feature/`, `fix/` или `docs/` до любых записываемых изменений, включая discovery/planning/log closure;
...
- наличие гейта текущей истины в generated product `AGENTS.md`, который требует task-ветку с типом `chore/`, `feature/`, `fix/` или `docs/` до любых записываемых изменений;
