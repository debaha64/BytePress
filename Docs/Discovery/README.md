...
- до ответов пользователя на discovery interview agent не переводит продукт в write-state: не меняет product truth, planning, logs и code;
- первый write-pass допускается только после подтверждённого discovery и отдельного task-веточного route;
