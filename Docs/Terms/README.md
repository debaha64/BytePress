...
## Bootstrap note
`BytePress` хранит канонические определения в карточках `TERM-*`.

Bootstrap-created product repo не копирует весь term-layer. Для него `Docs/Terms/Base_Terms.md` materialize компактный стартовый пакет терминов, экспортированный из канонических карточек `BytePress` и достаточный для раннего product-start.
