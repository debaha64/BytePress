...
`AGENTS.md` — единственное место машиночитаемой конфигурации устойчивого режима. Условное поле `SOT_GITHUB_REPOSITORY` существует только в `sot_github`; полный контракт режимов и переходов находится в [sops/sot.md](sops/sot.md).
...
3. Прочитать активный PLAN, `plans/backlog.md`, `plans/roadmap.md` и относящиеся к задаче SOP.
4. Определить фазу, точку контроля, `ALLOWED_SURFACES` и решения владельца, необходимые для ближайшего перехода.
5. Действовать только в границах активного PLAN и внешней границы задачи.

`SOT_MODE` читается до любых Git-команд. При `sot_files` Git CLI запрещён до отдельного перехода, утверждённого владельцем; отсутствие `.git` проверяется файловыми средствами.
...
| `roles/` | ответственность и полномочия основных ролей фаз SDLC |
| `skills/` | переносимые Agent Skills; собственные встроенные навыки отсутствуют |
...
| `tests/` | исполняемые модули технической и регрессионной проверки |
| `tools/` | исполняемые рабочие инструменты Product Unit |
| `src/` | код продукта |

## Обязательные границы

1. В `plans/active/` допускается от нуля до одного активного `PLAN-*.md`; при наличии PLAN агент обязан следовать ему.
2. Внешняя граница задачи может быть только сужена через `ALLOWED_SURFACES`.
3. `AGENTS.md`, `SYSTEM.md`, `sops/`, `roles/`, `skills/`, `templates/` и `tools/` защищены в `product-work`; точные переходные исключения перечислены в `SYSTEM.md`.
...
6. Инструменты Product Unit проверяют только локальный контур, не принимают решения и не выполняют автоматическое исправление.
...
6. [roles/README.md](roles/README.md) — соответствие `21/21` фаз и основных ролей.
7. [skills/README.md](skills/README.md) — граница стандартных Agent Skills и необязательное обнаружение клиентом.
