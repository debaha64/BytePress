...
### Текущее состояние
`BACK-000116` закрыт в `PLAN-000104`: базовый протокол управляемого прохода закреплён в `Pipeline/Workflows.md`, `Rules/Workflow.md`, плановом контуре и терминологическом слое. `ROAD-000045` остаётся `В_работе`, потому что следующие задачи этапа ещё не завершены.
