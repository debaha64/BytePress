...
## Навигация
- Первый этап: ROAD-000001 — Каркас репозитория и доменная структура
- Активный этап: отсутствует
- Следующий утверждённый этап: отсутствует
- Последний этап горизонта 0.4.0: ROAD-000050 — Предрелизная консолидация 0.4.0
- Последний завершённый этап: ROAD-000051 — Post-release 0.4.0: factual log, GitHub Release и archive

## Порядок записей
Записи `ROAD-*` идут хронологически от старых к новым. Новая запись добавляется в конец реестра этапов; верхняя навигация остаётся служебным статусом и не является дублем реестра.
...
### История
`ROAD-000001` ... `ROAD-000050` отражают завершённое развитие BytePress до фактического выпуска `0.4.0`. Их архивные `ROAD/PLAN` сохранены в `Plans/Archive/Releases/*.zip`.

`ROAD-000051` фиксирует post-release событие `0.4.0`: GitHub Release, factual release log, release archive `0.4.0.zip` и синхронизацию `develop`.

### Завершённый горизонт — BytePress 0.4.0
...
Горизонт `0.4.0` состоит из этапов `ROAD-000043` ... `ROAD-000050`. Roadmap удерживает этапы, цели, зависимости, историю и будущие горизонты; задачи активного и ближайшего этапа фиксируются в `Plans/Backlog.md`, а пошаговая реализация одной задачи — в отдельном `PLAN-*`.

### Будущий горизонт — BytePress 0.5.0
- Добавить mergeability gate перед release PR.
- Сделать создание GitHub Release после tag обязательной частью release route.
- Проверять видимость latest release на главной странице репозитория.
...
### Закрытие
Этап завершён в `PLAN-000091`. Задача `BACK-000102` сохранена в историческом release archive `Plans/Archive/Releases/0.3.0.zip`.
...
### Закрытие
Этап завершён в `PLAN-000092`. Задача `BACK-000103` сохранена в историческом release archive `Plans/Archive/Releases/0.3.0.zip`.
...
### Закрытие
Этап завершён в `PLAN-000093`. Задача `BACK-000104` сохранена в историческом release archive `Plans/Archive/Releases/0.3.0.zip`.
...
### Закрытие
Этап завершён в `PLAN-000094`. Задача `BACK-000105` сохранена в историческом release archive `Plans/Archive/Releases/0.3.0.zip`.
...
### Закрытие
Этап завершён в `PLAN-000095`. Задача `BACK-000106` сохранена в историческом release archive `Plans/Archive/Releases/0.3.0.zip`. Готовность выпуска `0.3.0` зафиксирована в `RL-000008`; фактический внешний выпуск, merge в `main` и tag остаются отдельными событиями.
...
### Закрытие
Этап завершён в `PLAN-000096`. Задача `BACK-000107` сохранена в историческом release archive `Plans/Archive/Releases/0.3.0.zip`. Фактический внешний выпуск `0.3.0` зафиксирован в `RL-000009`; готовность выпуска `RL-000008` сохранена как отдельное release-readiness событие.
...
Статус: Завершено
Связи: BACK-000124, BACK-000125, BACK-000126, BACK-000127, BACK-000128, BACK-000129, PLAN-000112, PLAN-000113, PLAN-000114, PLAN-000115, PLAN-000116, PLAN-000117, CHG-000124, CHG-000125, CHG-000126, CHG-000127, CHG-000128, CHG-000129, QL-000119, QL-000120, QL-000121, QL-000122, QL-000123, QL-000124
Источник: Запрос владельца от 2026-05-21
Дата_создания: 2026-05-21
...
Связанные_backlog: BACK-000124, BACK-000125, BACK-000126, BACK-000127, BACK-000128, BACK-000129
...
`BACK-000125` закрыт в `PLAN-000113`: manifest-договор release archive и zip-пакета закреплён без создания zip-архивов, без manifest конкретных версий и без запуска `ROAD-000048`. Следующий шаг внутри `ROAD-000047` — создать release manifest для исторических выпусков `0.1.0`, `0.2.0`, `0.3.0` по закреплённому шаблону.

`BACK-000126` закрыт в `PLAN-000114`: созданы release manifest для исторических выпусков `0.1.0`, `0.2.0` и `0.3.0` по `Plans/Archive/Releases/MANIFEST_TEMPLATE.md`. Зафиксированы только подтверждённые факты из `Logs/*`, архивных `ROAD/BACK/PLAN`, git tags и commit history; неподтверждённые факты отмечены как `не подтверждено`. Zip-архивы не создавались, `ROAD-000048` не начат.

`BACK-000127` закрыт в `PLAN-000115`: через `git archive` от tag созданы и проверены zip-пакеты `0.1.0`, `0.2.0` и `0.3.0`, manifest обновлены размером, путём и проверкой состава zip, `Plans/Archive/Releases/README.md` синхронизирован с фактическим состоянием архива.

`BACK-000128` закрыт в `PLAN-000116`: исправлён дефект `BACK-000127` без переписывания Git-истории. Ошибочные zip полного source tree удалены вместе с внешними каталогами release manifest; новые zip `0.1.0.zip`, `0.2.0.zip` и `0.3.0.zip` содержат только `MANIFEST.md`, архивный backlog и архивный plan релиза. `MANIFEST_TEMPLATE.md` перенесён в `Templates/Release_Manifest.md`.

`BACK-000129` закрыт в `PLAN-000117`: release zip переупакованы по подтверждённым ID-интервалам владельца. `0.1.0.zip` содержит `ROAD-000001` ... `ROAD-000015` и `PLAN-000001` ... `PLAN-000067`; `0.2.0.zip` содержит `ROAD-000016` и `PLAN-000068`; `0.3.0.zip` содержит `ROAD-000017` ... `ROAD-000042` и `PLAN-000069` ... `PLAN-000096`. Эти `ROAD/PLAN` физически удалены из открытых архивных каталогов; в текущем дереве остались `ROAD-000043+` и `PLAN-000097+`.

### Закрытие
`ROAD-000047` завершён после закрытия `BACK-000124`, `BACK-000125`, `BACK-000126`, `BACK-000127`, `BACK-000128` и исправляющего `BACK-000129`. Архивный контур разделён, manifest-договор закреплён, проверяемые zip-пакеты исторических выпусков содержат только manifest, архивные backlog и архивные plan по ID-интервалам. `ROAD-000048` остаётся утверждённым следующим этапом и не начат автоматически.
...
### Текущее состояние
`BACK-000130` закрыт в `PLAN-000118`: карты доменов и пользовательский setup-слой синхронизированы с фактической структурой после release archive. Корневой `Setup_Guide.md` оставлен только как переходный указатель, пользовательская настройка перенесена в `Docs/User/Setup_Guide.md`, а процессные и release-маршруты сведены к ссылкам на владельцев смысла.

`BACK-000131` закрыт в `PLAN-000119`: активные документы `Docs/User/*` нормализованы на русский технический язык, устаревшие англоязычные термины заменены принятыми терминами BytePress, ссылки на пользовательскую настройку ведут в `Docs/User/Setup_Guide.md`, а устаревшая ссылка на `ROAD-000012` удалена из активного пользовательского слоя. `ROAD-000049` не начат.
...
### Описание
Этап перед финальной предрелизной консолидацией: проверить порядок записей, шаблонную дисциплину реестров и журналов, а также индексы после завершения системных изменений горизонта `0.4.0`.

### Текущее состояние
`PLAN-000120` открыл `ROAD-000049` как отдельный этап перед финальной предрелизной консолидацией и перенёс финальную предрелизную консолидацию в `ROAD-000050`. `BACK-000115` остаётся утверждённой задачей этого этапа и не выполнен в корректирующем проходе. `ROAD-000050` не начат.

`BACK-000115` закрыт в `PLAN-000121`: порядок записей закреплён как хронологический от старых к новым, верхние полные ID-индексы реестров и журналов заменены краткой навигацией, шаблоны и схемы синхронизированы, а активные реестры и журналы проверены и приведены к договору порядка. `ROAD-000050` не начат.

### Закрытие
`ROAD-000049` завершён после закрытия `BACK-000115`. Этап порядка записей, шаблонов реестров и индексов закрыт; финальная предрелизная консолидация остаётся будущим утверждённым этапом `ROAD-000050` и не начата.
...
### Текущее состояние
`ROAD-000050` открыт и закрыт в `PLAN-000122` после финальной предрелизной консолидации. Проверены плановый контур, журналы, карты, активные документы, инструменты, release archive zip и минимальный generated product. Единственный подтверждённый дефект текущего слоя — generated wording `DoD` в `Tools/bp_bootstrap.py` — исправлен на `Определение_готовности`.

### Закрытие
`ROAD-000050` завершён после закрытия `BACK-000132`. Готовность выпуска `0.4.0` зафиксирована в `RL-000010`; внешний release PR в `main`, tag и изменение `origin/main` не выполнялись.
...
### Описание
Post-release этап после внешнего выпуска `0.4.0`: подтвердить merge `PR #137` в `main`, annotated tag `0.4.0`, GitHub Release `0.4.0`, factual `RL-000011`, release archive `0.4.0.zip` и очистку открытого архивного слоя от `ROAD-000043` ... `ROAD-000050` и `PLAN-000097` ... `PLAN-000122`.

### Закрытие
`ROAD-000051` завершён после закрытия `BACK-000133`. Факт внешнего выпуска `0.4.0` зафиксирован без изменения `origin/main`, без перемещения tag, без переписывания истории Git и без нового архитектурного решения. Будущие задачи `0.5.0` добавлены только как будущий горизонт без запуска отдельного плана.
