...
## Индекс
- ROAD-000036 — Завершающий проход перед выпуском 0.2.1: интервью, запрет догадок, язык и проверки
...
## ROAD-000036 — Завершающий проход перед выпуском 0.2.1: интервью, запрет догадок, язык и проверки
ID: ROAD-000036
Этап: Завершающий проход перед выпуском 0.2.1
Статус: В_работе
Связи: BACK-000101, PLAN-000089
Источник: Запрос владельца от 2026-05-02
Дата_создания: 2026-05-02
Дата_изменения: 2026-05-02
Цель: Закрыть оставшиеся дефекты свежего полевого теста Minesweeper, довести активный и создаваемый слой до чистого русского языка, упростить стартовое интервью, усилить запрет на догадки агента и подготовить BytePress к решению о выпуске 0.2.1.
Зависимости: ROAD-000035
Связанные_backlog: BACK-000101

### Описание
Этап закрывает один широкий завершающий проход. Стартовое интервью упрощено до блокирующих вопросов по классам `Результат`, `Граница`, `Вне границы`, `Ограничение`, `Стек и зависимости`, `Владение`, `Блокеры`; таймер, счётчик, несколько сценариев и расширения первой версии убраны из примеров. Запрет на догадки агента закреплён в `AGENTS.md`, `Pipeline/Workflows.md` и `Interview.md`: агент не предлагает дополнительный функциональный состав первой версии как пример, если пользователь его не назвал; гипотезы фиксируются отдельно и не становятся текущей истиной; пример результата использует только слова пользователя или уже подтверждённые требования. Языковая чистка активного и создаваемого слоя: `pass` → `проход`, `workflow` → `рабочий поток`, `gate` → `гейт`, `generated` → `создаваемый` в ключевых местах; английский оставлен только для имён файлов, каталогов, команд, веток, идентификаторов и имён собственных. Создаваемый продукт получает только `Docs`, `Plans`, `Logs`, `Pipeline`, `Tools`, `Templates`, `Schemas` и переходные `scripts`; запрещённые домены не появляются; `AGENTS.md` направляет в `Pipeline`; `Pipeline` содержит русские фазы, рабочие потоки и гейты; интервью не подсказывает лишний объём первой версии. `Minesweeper` не изменялся, новые домены не добавлялись, исторические архивы не переписывались.
