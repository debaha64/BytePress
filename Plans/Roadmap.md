...
- ROAD-000048 — Онтология, карты доменов и документационная чистка
...
### Средний горизонт
- Уточнить модель разработки продукта после стабилизации протокола прохода и проверок.
- Сформировать карту зрелости продукта на основе фактического опыта эксплуатации.
- Принять отдельное решение по zip-архивации release-пакетов, если это потребуется проверяемости выпусков.

### Дальний горизонт
- Рассмотреть тяжёлый CI только после отделения быстрых структурных проверок от проверок связности и пакета доказательств.
- Рассмотреть языковой линтер русского текста только после стабилизации терминологического контура.
- Рассмотреть отдельные домены `Ontology/*` или `Checks/*` только при наличии владельца, потребителя и проверки.
- Рассмотреть сложный pipeline engine только после подтверждения, что текущего repo-native контура недостаточно.
...
### Описание
Завершающий этап горизонта `0.4.0`: устранить дубликат `RULE-000017`, проверить индексы, ссылки, схемы, шаблоны и generated product после системных изменений, выполнить пакет проверок release-readiness и подготовить выпуск `0.4.0`.

---

## ROAD-000048 — Онтология, карты доменов и документационная чистка
ID: ROAD-000048
Этап: Онтология, карты доменов и документационная чистка
Статус: Утверждено
Связи:
Источник: Запрос владельца от 2026-05-21
Дата_создания: 2026-05-21
...
### Описание
Средний этап горизонта `0.4.0`: закрепить роль `AGENTS.md` как общей карты входа, уточнить карты доменов, разгрузить пользовательскую установку, привести Markdown-ссылки и связанные документы без запуска задач терминологического, проверочного или архивного этапов.
...
Статус: Утверждено
Связи: BACK-000111, BACK-000112, BACK-000113, BACK-000114
Источник: Запрос владельца от 2026-05-21
Дата_создания: 2026-05-21
Дата_изменения: 2026-05-21
