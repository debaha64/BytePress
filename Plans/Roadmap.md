...
### Текущее состояние
`BACK-000119` закрыт в `PLAN-000107`: уровни `lint`, `check`, `verify`, роль `bp_lint.py`, будущие роли `bp_check.py` и `bp_verify.py`, а также граница между автоматическими проверками, агентной смысловой проверкой и решением человека закреплены в проверочном и процессном договорах. `ROAD-000046` остаётся `В_работе`, потому что дальнейшие задачи этапа ещё не открыты и не завершены.
