...
## Zip-архивация
Zip-файлы в текущем проходе не создаются.
...
Если zip-архивация потребуется для post-release очистки, она выполняется отдельным следующим шагом внутри `ROAD-000047`: сначала создаётся manifest конкретного выпуска, затем проверяется состав package, и только после отдельного решения владельца допускается сохранение крупного бинарного zip в репозитории.

## Следующий проход
Следующий проход `ROAD-000047` должен создать release manifest для исторических выпусков `0.1.0`, `0.2.0` и `0.3.0` по этому договору. Этот проход не создаёт manifest конкретных версий и не создаёт zip-архивы.
