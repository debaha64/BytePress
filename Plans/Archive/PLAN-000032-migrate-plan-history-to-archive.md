...
2. Ввести active/archive layout для plan-layer.
   - Описание: Создать `Plans/Archive/`, перевести historical `BP-*` и завершённые `PLAN-*` в `Plans/Archive/PLAN-*`, а в активном `Plans/` оставить только текущий `Plan`.
...
- historical `BP-*` plan-files migrated в `Plans/Archive/PLAN-*`.
